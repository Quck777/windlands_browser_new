...
$t1 = $db->sqla("SELECT * FROM quest WHERE id = ".TOUR3."");
if(empty($http->get["show_t3"]) and !$t1["finished"]) // Турнир 1 15-99 10x10
{
	$tour3 .= "<a class=wbut href='main.php?show_t3=1'>Турнир 3</a>";
}
if(@$http->get["show_t3"] and !$t1["finished"]) // Турнир 1 15-99 10x10
{
...
if(@$http->get["show_t3"] and !$t1["finished"] and $t1["type"]==0) // Турнир 1 15-99 10x10
{
	if(@$http->get["tour3"]=="join" and $player->pers["level"]>=15 and $player->pers["tour"]==0)
	{
		set_vars("tour=3",UID);
		$player->pers["tour"]=3;
	}
	if(!mtrunc(T_START+$t1["time"]-tme()))
	{
		...
		$db->sql("UPDATE quest SET finished=0,time=".tme()."+".T_START.",type=1,lParam=".$id." WHERE id = ".TOUR3);
	}
	else
	{
		...
		$users = $db->sql("SELECT * FROM users WHERE tour=3;");
		...
		echo "<i>Описание:</i> Начинается классический, хаотический бой 10х10 между участниками турнира.
			...
			<br><i>Призы:</i> 30 пергаментов, 2000 LN, 200000 опыта";
		echo "</td></tr></table>";

		if($player->pers["level"]>=15 and $player->pers["tour"]==0)
...
elseif(@$http->get["show_t3"] and ($t1["type"]==1 or $t1["type"]==2) and !$t1["finished"])
{
...
elseif($t1["type"]==3 and !$t1["finished"])
	$db->sql("UPDATE quest SET finished=1,time=".tme()." WHERE id=".TOUR3);
?>