...
<?php
	...
	($r==$tunnel["r3id"] and $k=$tunnel["r3k"]))
	{
		...
		if ($inst2["id"])
			$kk = mtrunc(floor((rand($inst["udmin"],$inst["udmax"])*sqrt($player->pers["sp12"]*100)/350)/sqrt($r["price"]*4) + rand(2,3)));
		else
			$kk = mtrunc(floor((rand($inst["udmin"],$inst["udmax"])*sqrt($player->pers["sp12"]*100)/700)/sqrt($r["price"]*4) + rand(1,2)));
		$mdur = mtrunc($instp*$kk*($r["price"]/mtrunc(sqrt($player->pers["sp12"]/50)+1)))+1;
		...
		if (rand(1,500)==1)
		{
			...
			if ($v)
			{
				...
				$db->sql("UPDATE wp SET durability=durability-".($kk/2+1)." WHERE id='".$inst["id"]."'", __FILE__,__LINE__,__FUNCTION__,__CLASS__);
		...
		if ($za==0)
		{
			...
			{
				if ($kk>$r["k"]) $kk=$r["k"];
				$r_get .= "<font class=user>[".$r["name"]."]</font> : <b>".$kk."</b> ед.<br><font class=timef>".$kk*$r["price"]." <img src=images/money.gif></font><br>Долговечность кирки понизилась на ".round($mdur).".<br> Мирный опыт +3.<br>Добыча камней +".round((10/($player->pers["sp12"]+1)),3).".<br>Шахтёрство +".round((8/($player->pers["sp7"]+1)),3).".";
				...
				$player->pers["waiter"]=$t+$timed;
				$db->sql("UPDATE wp SET durability=durability-".($mdur)." WHERE id='".$inst["id"]."'", __FILE__,__LINE__,__FUNCTION__,__CLASS__);
				set_vars("sp12=sp12+".round((10/($player->pers["sp12"]+1)),3).",sp7=sp7+".round((8/($player->pers["sp7"]+1)),3).",peace_exp=peace_exp+3,waiter=".$player->pers["waiter"].",tire=tire+10",$player->pers["uid"]);
				...
				else
				{
					$db->sql("INSERT INTO `wp` ( `id` , `uidp` , `weared` ,`id_in_w`, `price` , `dprice` , `image` , `index` , `type` , `stype` , `name` , `describe` , `weight` , `where_buy` , `max_durability` , `durability` ,`p_type`)
						VALUES (0, '".$player->pers["uid"]."', '0','res..".$r["image"]."','".$kk*$r["price"]."', '0', 'resources/".$r["image"]."', '0', 'resources', 'resources', '".$r["name"]."', '', '".$kk."', '0', '".$kk."', '".$kk."','7');", __FILE__,__LINE__,__FUNCTION__,__CLASS__);
				}
			...
			{
				$r_get .= "Неудачным ударом кирки, вы испортили ресурс.<br>Долговечность кирки понизилась на ".round($kk/2+1).".<br> Мирный опыт +1.<br>Добыча камней -".round((5/($player->pers["sp12"]+1)),3).".<hr><i>Совет: Это происходит из-за слишком малого количества вашего умения \"Добыча камней\". Поднять это умение можно в университете.</i><hr>";
				$player->pers["waiter"]=$t+$timed;
				$mm = round((5/($player->pers["sp12"]+1)),3);
				if ($player->pers["sp12"]<1) $mm=0;
				$db->sql("UPDATE wp SET durability=durability-".($kk/2+1)." WHERE id='".$inst["id"]."'");
