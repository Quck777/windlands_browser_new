...
if ( !empty($v['id']) )
{
	...
	if ( !empty($v["name"]) )
	{
		...
		if ($v['dprice']==0)
		{
			if ($v["where_buy"]==0){ $val = 'LN'; $img='money.gif';}
			elseif ($v["where_buy"]==4){ $val = 'IM'; $img='gameplay/im.png';}
			else { $val = 'LN'; $img='money.gif';}
		...
		else $text .= "<img src=/images/signs/diler.gif> <b>".$v["dprice"]." Бр. </b><font class=items><br>";
		...
		if ($rank_i>0) $attrs.= '<tr><td>Ранк: </td><td><i>'.$rank_i.'</i></td></tr>';
		$attrs .= '</table>';
		if (!$v_count) $attrs = '';
