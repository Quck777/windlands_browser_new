...
function punish($persto,$perswho,$duration,$reason)
{
	...
	{
		echo 'C персонажа <b>'.$persto['user'].'</b> снято заклинание кары смотрителей (<b>'.$perswho['user'].'</b>)';
		say_to_chat('z','C персонажа <b>'.$tologn.'</b> снято заклинание кары Смотрителей (<b>'.$ispoln.'</b>).',0,'','*',0); 
...
function benediction($persto,$perswho,$duration,$reason)
{
	...
	if ( $duration>-1 and ($aur_wt[0]==false or $aur_wt[0]<$persto['lastom']) )
	{
		...
		$a['esttime'] = $duration*60;
		$a['name'] = 'Благословление Смотрителей';
...
function diler($persto,$perswho,$count,$koment)
{
	...
	if ($count>0)
	{
		...
		say_to_chat('s','Персонаж <b>'.$perswho['user'].'</b> (Официальный дилер проекта) продал вам <b>'.$count.' Бр.</b>',1,$persto['user'],'*',0); 
